...
Можно направить и **отдельный сайт** в свой чат (приоритетнее чата страны) —
переменной `TARGET_CHAT_SITE_<КЛЮЧ>` (ключи см. в `/sites`):
```env
TARGET_CHAT_SITE_FLRU=6559307546
TARGET_CHAT_SITE_FREELANCESPACE=6559307546
```
Приоритет доставки: **свой чат сайта → чат страны → общий чат**.
